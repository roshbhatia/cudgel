...
use cudgel::{
    embeddings::{FallbackTokenizer, TokenizerStrategy},
    Config,
};
...
fn test_fallback_initialization() {
    ...
    tokenizer
        .validate()
        .expect("Tokenizer should be valid after initialization");
}
...
fn test_fallback_encodes_simple_code() {
    ...
    let embedding = tokenizer.encode(code).expect("Failed to encode code");
...
fn test_fallback_similar_code_has_high_similarity() {
    ...
    assert!(
        similarity > 0.6,
        "Similar code should have cosine similarity > 0.6, got: {}",
        similarity
    );
}
