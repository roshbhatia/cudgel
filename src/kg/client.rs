...
impl KgClient for PostgresKgClient {
    ...
    async fn get_components(&self, repo_id: &RecordId) -> Result<Vec<Component>> {
        ...
        Ok(rows
            ...
            .map(|r| {
                ...
                let created_naive: NaiveDateTime = r.get(6);
                let updated_naive: NaiveDateTime = r.get(7);
    ...
    async fn get_entity(&self, entity_id: &RecordId) -> Result<Option<CodeEntity>> {
        ...
        Ok(row.map(|r| {
            ...
            use chrono::{NaiveDateTime, TimeZone, Utc};
            let created_naive: NaiveDateTime = r.get(13);
            let updated_naive: NaiveDateTime = r.get(14);
        ...
        Ok(rows
            ...
            .map(|r| {
                ...
                CodeEntity {
                    ...
                    metadata,
                    summary: r.get(11),
                    created_at: Utc.from_utc_datetime(&r.get::<_, chrono::NaiveDateTime>(13)),
                    updated_at: Utc.from_utc_datetime(&r.get::<_, chrono::NaiveDateTime>(14)),
                }
            })
            .collect())
    }
    ...
    async fn search_entities_by_name(
        ...
        let rows = client
            .query(
                ...
                FROM kg_entities e
                JOIN kg_components c ON e.kg_component_id = c.id
                WHERE c.kg_repository_id = $2 
                  AND e.name ILIKE CONCAT('%', $1, '%')
                  AND CASE 
                      WHEN e.name ILIKE $1 THEN 1.0
                      WHEN e.name ILIKE CONCAT('%', $1, '%') THEN 0.8
                      ELSE 0.5
